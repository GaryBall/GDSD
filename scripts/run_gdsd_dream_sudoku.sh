...
TLC_TOPK=32
REMASKING="random"
# The bundled 4x4 sudoku set has 1e6 puzzles, so one epoch would be ~62k optimizer steps at
# this batch size. Cap the run explicitly instead of relying on num_train_epochs.
MAX_STEPS=10000
...
accelerate launch \
  ...
  gdsd/gdsd_train.py \
  --config scripts/train_gdsd.yaml \
