...
class GDSDTrainer(GRPOTrainer):
    ...
    def _compute_denoise_probs_from_masked_batch(
        ...
        # Ablation baseline for GDSD-TLC: no token-level logit centralization,
        # i.e. read out the plain normalized log-probability log p(y).
        # Everything else (ELBO 1/p_mask weighting, coupled branch, averaging)
        # matches GDSDTLCTrainer so that TLC is the only difference.
        ...
        coupled_logp_bar = coupled_logp_tok
